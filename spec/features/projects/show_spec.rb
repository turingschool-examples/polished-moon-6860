...
RSpec.describe 'project show page' do
  before (:each) do
    ...
    @item2 = @project1.items.create!(name: "Bed frame", cost: 340)
  ...
  it 'shows the average cost per item needed for the project' do
    ...
    expect(page).to have_content(@project1.avg_item_price)
    expect(page).not_to have_content(@item2.name)

    visit "/projects/#{@project2.id}"
    expect(page).to have_content(@project2.name)
    expect(page).to have_content(@project2.avg_item_price)
