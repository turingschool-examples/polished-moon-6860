...
RSpec.describe Item, type: :model do
  ...
  describe 'methods' do
    ...
    it '#manufacturer_count' do
      expect(@item_3.manufacturer_count).to eq(1)
      expect(@item_16.manufacturer_count).to eq(2)
      expect(@item_6.manufacturer_count).to eq(2)
      expect(@item_7.manufacturer_count).to eq(1)
    end
  end
end
